...
use simble::device::classic_host::{
    RfcommHandler, SdpQueryHandler, SharedRfcommPort, SharedSdpQueryResults, inquiry_mode,
    scan_enable,
};
...
impl Initiator {
    fn new(name: &str, class_of_device: [u8; 3], target: Address) -> Self {
...
fn main() {
    ...
    let mut initiator = Initiator::new(&name, [0x0C, 0x02, 0x5A], target);
