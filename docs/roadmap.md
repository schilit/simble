...
| H3 | **Typed zerocopy host parsing.** | **partly done** (7a4fa3d, scan-report AD; SMP key-distribution) | A full survey settled the picture: the transport/host layer is done — `packets/{hci_events,ext_adv,big,iso,l2cap_signaling,att}`, `client/gatt_client`, btsnoop (`netsim.rs`), `scan_report.rs` AD decode, and now the SMP pairing path (`MasterIdentification` / `IdentityAddressInformation` typed, EDIV as `U16`) are all typed views. The genuine remaining hand-rolled inbound parsing is **profile/protocol code**, not transport: ranked `classic/sdp.rs` (SDP PDUs), `device/classic_host.rs` (inquiry-result `cod_offset`, security events — no typed `HciEvent` variants), `classic/avrcp.rs` (BE cursor), `profiles/{ancs,asha,bass}.rs`, `classic/{a2dp,avdtp}.rs`. Transport length-field reads (`rootcanal.rs`, `usb.rs` framing) are legitimately hand-rolled — leave. This wider surface is a separate, larger effort; not scheduled. |
