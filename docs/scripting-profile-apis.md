...
control-point state machine; a Scan Delegator parses Add Source and
republishes a Receive State; CSIP has set-membership crypto. That behaviour
exists in Rust, in `src/profiles/`, tested — and a script cannot reach it.

**17 of 20 profiles with a `register()` have no script binding.** The three
that do are `add_pacs`, `add_ascs`, `add_ras`, in
`wasm_ws.rs::register_web_extensions`, and their doc comment states the
...
simulator and a script is not a Play Store app. The Android shape buys one
thing — a developer reading a script recognises what it does. Permission
annotations are Android's deployment policy and have no bearing here.

This also corrects an earlier argument: that Auracast could not be a script
because Rhai binds no HCI types. It does not need them.
`android::BluetoothGattServer` exposes no ATT PDUs either — the Rust binding
does that work underneath. A profile binding hides the layer below it; that is
what a profile is.
...
Wraps [`crate::device::BigBroadcaster`], verified against Bumble in both
directions (`tests/interop/auracast_*.py`).
...
Pure GATT underneath: it writes BASS Add Source and reads the Broadcast
Receive State back. Nothing below GATT is involved.
...
This pair matters most: it makes the Assistant↔Delegator conversation
scriptable on *both* sides, which turns a scene into a test. It is also the
case that would have caught `bass.rs` reporting `SynchronizedToPa`
unconditionally — a scripted Assistant asserting that state against a Delegator
that always said it.
...
> that speaks BR/EDR and a scene slot to put a device in — and both now exist,
> so its bindings are the easy part too.

### LE — a binding is the whole job

`SceneEngine` hosts four things: `add_peripheral`, `add_scanner`, `add_central`,
`add_scripted_central`. Every LE device is one of those, so the Rust already
has somewhere to live and the only missing piece is a name a script can say.
...
A2DP, AVRCP, HFP and Classic HID are **not** blocked by a missing Rhai
binding. They were blocked by two things below it, both now done:

1. **The simulated controller spoke only LE.** This was the real blocker.
   `SceneEngine`'s `Link` (`controller/sim.rs`) had no inquiry, no scan enable,
   no paging and no BR/EDR Connection Complete — so even with a scene slot a
   `ClassicHost` would have sat idle for ever: it consumes H4 and emits H4, and
   nothing was ever going to send it a Connection Request. `sim.rs` now models
   scan enable, inquiry, paging, Remote Name Request and classic ACL routing.
2. **The scene could not host a BR/EDR device.**
   `SceneEngine::add_classic_device` is now the fifth thing a scene can host,
   beside `add_peripheral`, `add_scanner`, `add_central` and
   `add_scripted_central`.

Two `ClassicHost`s in one scene now inquire, page, open L2CAP, query SDP, open
the advertised RFCOMM channel and exchange data with no netsim and no radio —
see `classic_scene_tests` in `transport/wasm_ws.rs`.

`ClassicHost`'s constructor does *not* take an `SdpServer`: it is
`ClassicHost::new(name, class_of_device)`, and handlers — `SdpHandler`,
`RfcommHandler`, and anything else implementing `ProtocolHandler` — are
registered separately with `register_handler`. And `controller/lmp.rs` models
LMP, which is controller-to-controller *below* HCI, so it produces no
host-facing events. It is now used inside `sim.rs` for the connection handshake
(with a new host-gated `ConnectionPending` state, because answering a page is
the host's decision), but it was never the missing layer.

**What is left for the profile bindings** is genuinely the binding work. The
`ProtocolHandler` half is done for three of the four: `device::a2dp`
...
`ProtocolHandler` gained what those needed: `psms()` so one handler can claim
Control **and** Interrupt, `on_channel_data(HandlerChannel, ..)` so a handler is
told which channel spoke, and `poll_channel_requests()` so AVDTP can ask the
host for its media transport channel. Single-PSM handlers were not touched.

### Android names for the Classic surface

When those bindings are written, they should use Android's API set, exactly as
the LE ones do — `@SystemApi` is not a reason to avoid a name.

The **Rust type** column is what a binding would wrap. It is filled in only
where the handler exists; an empty cell means there is nothing yet to bind.
...
`BluetoothAvrcpController` is the closest Android has to `AvrcpController`, and
it is not close. It is the proxy for Android acting as an AVRCP *controller* —
car-kit mode, paired with a phone that holds the player — the same role
`AvrcpController` plays. But it is **deprecated** and its public surface is two
methods, `getConnectedDevices()` and `getConnectionState()`. There is no
`play()`, `pause()`, or `getPlayStatus()`: on Android those live behind
`MediaController` / `MediaSession`, a **media framework** API, not a Bluetooth
one, and the AVRCP transport underneath is not addressable from an app at all.
So a binding named `BluetoothAvrcpController` exposing `play` and `pause` would
invent exactly the plausible-looking API rule 5 forbids. Two honest options:
keep Android's name and expose only what Android exposes, putting the transport
keys on a `MediaController`-shaped companion; or say in the doc comment that the
name is borrowed for the *role* and the methods have no Android counterpart.

`AvrcpTarget` has **no Android proxy at all**. Android is the target whenever
it is the phone streaming A2DP, but it reaches that role through `MediaSession`
— a media API published over Bluetooth by the system — and there is no
`BluetoothAvrcpTarget` to mirror. The cell is empty rather than a guess.

One name needs care, and `src/scripting/hid.rs`'s module doc is the long
version: **`BluetoothHidHost` is not Classic-only.** Android's proxy spans both
transports — `HidHostService.java` has no transport-specific code, and the
split happens down in `bta/hh/`. The existing `android::BluetoothHidHost`
binding is the HOGP (LE) half; `ClassicHidHost` is the *same role on the other
transport*, and the `Classic` prefix exists because Rust needs two names, not
because Bluetooth has two roles. A binding that covers both should be one
binding.

The one that maps onto the **controller** layer built here is not a profile
proxy at all. Android's discovery surface is
`BluetoothAdapter.startDiscovery()` with the `ACTION_FOUND` broadcast — HCI
Inquiry plus Inquiry Result — and the remote name is
`BluetoothDevice.getName()`, HCI Remote Name Request. Both are now real
underneath.
...
- **Classic profile bindings.** The architectural blocker is gone — a scene
  ...
  passing scene tests. What is left is the naming exercise, and the table above
  is the answer.
- **Permissions, of any kind.** They are Android's deployment policy, not an API
  shape, and do not apply to a simulator.
