...
//! Bluetooth SIG assigned-number names for 16-bit UUIDs, generated from the
//! SIG's Assigned Numbers repository
//! (<https://bitbucket.org/bluetooth-SIG/public>, `assigned_numbers/uuids/`
//! `service_uuids.yaml` / `characteristic_uuids.yaml` / `descriptors.yaml`,
//! fetched 2026-08-21). The source data is published by
//! Bluetooth SIG "AS IS" per the notice in those files; the names are
//! reproduced here solely to label UUIDs in tool output. Regenerate by
//! re-running the extraction over fresh copies of the YAML files (see the
//! `lookup` tool's docs) — do not edit by hand.

/// SIG-assigned service names, sorted by 16-bit UUID.
pub static SERVICE_NAMES: &[(u16, &str)] = &[
    (0x1800, "GAP"),
    (0x1801, "GATT"),
...
/// SIG-assigned characteristic names, sorted by 16-bit UUID.
pub static CHARACTERISTIC_NAMES: &[(u16, &str)] = &[
    (0x2A00, "Device Name"),
    (0x2A01, "Appearance"),
...
/// SIG-assigned descriptor names, sorted by 16-bit UUID.
pub static DESCRIPTOR_NAMES: &[(u16, &str)] = &[
...
/// Looks a 16-bit UUID up across all three tables; returns the kind
/// ("service" / "characteristic" / "descriptor") and the SIG name.
pub fn name_of(uuid16: u16) -> Option<(&'static str, &'static str)> {
    for (kind, table) in [
        ("service", SERVICE_NAMES),
        ("characteristic", CHARACTERISTIC_NAMES),
        ("descriptor", DESCRIPTOR_NAMES),
    ] {
        if let Ok(i) = table.binary_search_by_key(&uuid16, |&(u, _)| u) {
            return Some((kind, table[i].1));
        }
    }
    None
}
